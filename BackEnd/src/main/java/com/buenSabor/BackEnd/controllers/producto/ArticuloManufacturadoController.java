...
public class ArticuloManufacturadoController extends
    ...
    @Operation(summary = "Crear un nuevo artículo manufacturado", description = "Crea un artículo manufacturado con sus insumos y detalles")
    @ApiResponses(value = {
    ...
    public ResponseEntity<?> saveFromDTO(
            @Parameter(description = "Datos del manufacturado a crear", required = true) @Valid @RequestBody ArticuloManufacturadoDTO dto) {
        try {
    ...
    @Operation(summary = "Actualizar un artículo manufacturado", description = "Actualiza los datos de un manufacturado existente")
    @ApiResponses(value = {
    ...
    public ResponseEntity<?> updateFromDTO(
            @Parameter(description = "ID del manufacturado a actualizar", required = true) @PathVariable Long id,
            @Parameter(description = "Datos actualizados del manufacturado", required = true) @Valid @RequestBody ArticuloManufacturadoDTO dto) {
        try {
            logger.info("Actualizando manufacturado con ID: {}", id);
            ArticuloManufacturadoDTO updatedDTO = articuloManufacturadoService.actualizarManufacturado(id, dto);
    ...
    @Operation(summary = "Eliminar un manufacturado (soft delete)", description = "Realiza una eliminación lógica marcando existe=false")
    @ApiResponses(value = {
    ...
    public ResponseEntity<?> eliminar(
            @Parameter(description = "ID del manufacturado a eliminar", required = true) @PathVariable Long id) {
        try {
    ...
    @Operation(summary = "Listar manufacturados activos", description = "Retorna todos los manufacturados marcados como existentes")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Buscar manufacturados por subcategoría", description = "Retorna manufacturados filtrados por ID de subcategoría")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Lista obtenida exitosamente"),
            @ApiResponse(responseCode = "500", description = "Error interno del servidor")
    })
    ...
    public ResponseEntity<?> getManufacturadoBySubcategoria(
            @Parameter(description = "ID de la subcategoría", required = true) @PathVariable Long idSubcategoria) {
        try {
    ...
    @Operation(summary = "Buscar manufacturados por nombre", description = "Retorna manufacturados que contengan el texto en su nombre")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Lista obtenida exitosamente"),
            @ApiResponse(responseCode = "500", description = "Error interno del servidor")
    })
